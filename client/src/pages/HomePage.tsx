...
const FEATURES = [
  { icon: <Zap size={18} />, title: 'Instant feedback', desc: 'Powered by Gemini 2.5 Flash. Paste code, get a detailed review in seconds.' },
  { icon: <ShieldCheck size={18} />, title: 'Security analysis', desc: 'Detects SQL injection, XSS, auth flaws, and 50+ vulnerability patterns.' },
  { icon: <BarChart3 size={18} />, title: 'Quality scoring', desc: 'A 0–100 quality score with per-category breakdown and fix suggestions.' },
  { icon: <Globe size={18} />, title: '15+ languages', desc: 'JavaScript, TypeScript, Python, Go, Rust, Java, C++, PHP, Ruby, Swift & more.' },
]
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-brand-bg">
      ...
      {/* ── Hero ─────────────────────────────────────────────── */}
      <section className="relative pt-32 pb-28 px-5 text-center overflow-hidden">
        {/* Orange glow */}
        <div className="pointer-events-none absolute inset-0"
          style={{ background: 'radial-gradient(ellipse 55% 45% at 75% 15%, rgba(224,122,82,0.12), transparent)' }} />
        ...
        <div className="relative max-w-2xl mx-auto animate-fade-up">
          ...
          <h1 className="heading-serif text-5xl sm:text-6xl md:text-7xl text-brand-text mt-5 mb-5">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link to="/review" className="btn-primary px-7 py-3 text-sm sm:text-base">
              Start reviewing free
              <ArrowRight size={16} />
            </Link>
            {!isAuthenticated && (
              <a href={`${BASE}/api/auth/github`} className="btn-outline px-7 py-3 text-sm sm:text-base">
                <Github size={16} />
                Sign in with GitHub
              </a>
            )}
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 mt-10">
            {['Security Analysis', 'Line Numbers', 'Fix Suggestions', 'Quality Score', '15+ Languages'].map(tag => (
              <span key={tag} className="text-xs font-medium text-brand-muted border border-brand-border bg-brand-surface px-3 py-1 rounded-full">
                {tag}
              </span>
            ))}
          </div>
      ...
      {/* ── Divider ──────────────────────────────────────────── */}
      <div className="max-w-5xl mx-auto px-5"><div className="border-t border-brand-border" /></div>

      {/* ── Sample Review ─────────────────────────────────────── */}
      <section className="py-20 px-5">
        <div className="max-w-3xl mx-auto">
          <span className="section-label">See it in action</span>
          <h2 className="heading-serif text-3xl sm:text-4xl text-brand-text mb-8">Real feedback, instantly.</h2>
  ...
  return result;
              ...
              {SAMPLE_ISSUES.map((issue, i) => (
                <div key={i} className="flex items-start gap-3 px-5 py-3.5">
                  <span className="mt-px text-sm flex-shrink-0">
                    {issue.type === 'critical' ? '🔴' : issue.type === 'warning' ? '🟡' : issue.type === 'info' ? '🔵' : '✅'}
                  </span>
      ...
      <section className="py-20 px-5">
        <div className="max-w-5xl mx-auto">
          ...
          <h2 className="heading-serif text-3xl sm:text-4xl text-brand-text mb-10">Everything you need.</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {FEATURES.map((f, i) => (
              <div key={i} className="card-hover p-5">
                <div className="w-9 h-9 rounded-lg border border-brand-border bg-brand-surface-2 flex items-center justify-center text-brand-accent mb-4">
                  {f.icon}
                </div>
      ...
      <section className="py-20 px-5">
        <div className="max-w-5xl mx-auto">
          <div className="relative card p-10 sm:p-14 text-center overflow-hidden">
            <div className="pointer-events-none absolute inset-0"
              style={{ background: 'radial-gradient(ellipse 60% 80% at 50% 120%, rgba(224,122,82,0.1), transparent)' }} />
            <div className="relative">
              <h2 className="heading-serif text-3xl sm:text-4xl text-brand-text mb-4">Ready to review your code?</h2>
              ...
              <Link to="/review" className="btn-primary px-7 py-3 text-base">
                Start reviewing free <ArrowRight size={17} />
              </Link>
            </div>
          </div>
        </div>
      ...
      <footer className="border-t border-brand-border bg-brand-surface">
        <div className="max-w-5xl mx-auto px-5 py-10">
          <div className="flex flex-col sm:flex-row items-start justify-between gap-8">
            ...
            <div>
              ...
              <p className="text-xs text-brand-muted leading-relaxed max-w-xs">
                Free AI-powered code review. Instant feedback on security, performance, and code quality.
              </p>
            </div>
            ...
            <div className="flex flex-col sm:items-end gap-3">
              <p className="text-xs text-brand-muted">
                Built by{' '}
                <a href="https://www.linkedin.com/in/anshkrgupta/" target="_blank" rel="noopener noreferrer"
              ...
              <div className="flex items-center gap-3">
                ...
                <a href="https://github.com/AnshGupta-byte" target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1.5 text-xs text-brand-muted hover:text-brand-accent transition-colors">
                  <Github size={13} /> GitHub
                </a>
                <span className="text-brand-border">·</span>
                <a href="mailto:anshg397@gmail.com"
                  className="flex items-center gap-1.5 text-xs text-brand-muted hover:text-brand-accent transition-colors">
          ...
          <div className="border-t border-brand-border mt-8 pt-5 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-brand-muted">
            ...
            <Link to="/review" className="hover:text-brand-text transition-colors">Start reviewing →</Link>
          </div>
        </div>
      </footer>
